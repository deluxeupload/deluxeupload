...
if(!isset($_GET['fileid'])){
    header("Location:error.php?err=Invalid link");
    exit();
}
...
if(!$link){
    header("Location:error.php?err=Invalid or expired link");
    exit();
}

if(strtotime($link['expires_at']) < time()){
    header("Location:error.php?err=This link has expired");
    exit();
}
...
<body lang="en">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <button type="button" class="btn qrcodeButton" onclick="create_qr_code('<?= $_SERVER['SERVER_NAME'].$urlname ?>')" title="Click for create qr code for file."> Create <i class="fas fa-qrcode"></i></button>
                    <div class="prepareQr">
                        <div id="timer_qr" class="container bg-secondary p-3 text-light mt-2 mb-2"></div>
                        <div id="qrcodeLink" class="mt-2 mb-2"></div>
                    </div>
                    <p id="para_qr" class="text-center"></p>
                </div>
    ...
    <div class="container dv1 mt-3 mb-3">
        <div class="card mx-auto">
            <div class="card-body">
                <h2><?= htmlspecialchars($file['file_name']); ?></h2>
                <a href="reports.php?id=<?= htmlspecialchars($file['user_id']); ?>&fileid=<?= $file['id']; ?>" style="color: red;text-decoration:none;"><i class="fas fa-flag"></i> Report abuse</a>
                <hr>
                <p><?= "Size : " . number_format($file['file_size'] / (1024 * 1024), 2) . "MB"; ?></p>
                <hr>
                <p>Download Count <?= htmlspecialchars($file['download_count']); ?></p>
                <hr>
                <p>Uploaded at <?= htmlspecialchars($file['upload_date']); ?> by <?= htmlspecialchars($user['userName']); ?></p>
                <hr>
                ...
                <hr>
                <a type="button" href="download.php?id=<?= $file['id']; ?>" class="btn p-3 w-100" id="download">Download</a>
                <div id="prepare_link" class="container bg-secondary p-3 text-light mt-2 mb-2"><span id="timer_download" style="cursor:wait;">Wait a few seconds 5s</span></div>
                <div class="mt-2 mb-2 d-flex flex-direction-row justify-content-center align-items-center dv3">
                    <input type="text" class="form-control" value="<?= $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI']; ?>" readonly>
                    <a href="" title="Copy link" onclick="copyToClipboard(event,'<?= $_SERVER['SERVER_NAME'].$urlname ?>')" class="link">
                        <i class="fas fa-copy"></i>
                    </a>
                    <a href="" title="Create QRCode" data-bs-target="#qrcode" data-bs-toggle="modal" class="link">
